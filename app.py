...
def get_account_credentials(region: str) -> str:
    r = region.upper()
    if r == "IND":
        return "uid=4228117491&password=JOBAYAR_CODX-ZYQ0EM9OV"
    elif r == "BD":
        return "4230245525=uid&password=XRSUPER-KJDPCIFRX"
    elif r in {"BR", "US", "SAC", "NA"}:
        return "uid=4229921018&password=BY_MOOHA-EIIQSGVK2-MOOHA"
    else:
        return "uid=4229921018&password=BY_MOOHA-EIIQSGVK2-MOOHA"
...
async def get_region_by_uid(uid: str) -> str:
    ...
    async with httpx.AsyncClient() as client:
        resp = await client.get(f"https://jnl-codex-region.vercel.app/region?uid={uid}")
